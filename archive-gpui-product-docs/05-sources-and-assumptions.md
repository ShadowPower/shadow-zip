...
资料核验覆盖 GPUI 与 Zed、minizip-ng、sevenz-rust2、libarchive、unrar.rs、7-Zip-zstd、NanaZip、7-Zip SDK、RARLAB 资料、图片解码库以及平台集成候选库。由于相关项目在持续演进，进入开发阶段后应定期重新核对关键依赖版本和许可证条款。

## 2. GPUI 与三平台能力

GPUI 的 README 仍明确表明其处于 pre-1.0 状态，并提示 API 可能发生 breaking changes。这意味着项目不能将 GPUI 视为完全稳定的长期 API。UI 层必须被封装，业务逻辑不应直接依赖 GPUI 的细节对象和临时模式。

另一方面，Zed 主仓库已经展示 Windows、Linux 和 macOS 的安装与构建路径，并且 Zed 本身以 GPUI 作为核心 UI 技术。这说明最新 GPUI 生态已经具备三平台桌面应用构建基础。综合两方面资料，可以得出正式判断：GPUI 可以作为该产品的 UI 技术路线，但其文档、API 稳定性和平台细节仍然构成工程风险。

相关资料包括 GPUI README、Zed 主仓库、Zed releases 以及 gpui-component。gpui-component 提供虚拟化列表、表格和其他组件能力，对归档工具的大文件列表场景具有参考价值。
...
本方案包含若干需要原型验证的工程假设。第一，GPUI 可以在三平台上支撑高密度工具型界面和大列表虚拟化。第二，sevenz-rust2 对常见 7z 扩展 codec 的支持足以作为主路径。第三，minizip-ng 的三平台构建和 FFI 维护成本可控。第四，tar.zst 单遍流式任务可以在保持取消响应的同时获得良好吞吐。第五，图片预览的内存和磁盘策略可以通过分级加载与 LRU 缓存控制在合理范围内。

这些假设都不能仅凭文档最终确认。进入 Phase 0 后，应通过原型、样本库和性能基线逐项验证。

## 9. 许可证关注点

许可证审计必须在稳定版本前完成。重点包括 GPUI/Zed 相关许可证、minizip-ng、sevenz-rust2、libarchive、UnRAR、RAR CLI、7-Zip SDK、7-Zip-zstd、图片 codec 依赖、静态链接和动态链接义务、third-party notices 以及安装包中 helper 的分发条款。

其中 RAR 创建是最敏感事项。产品不得在未完成授权确认前承诺内建 RAR 创建能力。UnRAR 的读取能力也应确认其分发条款，避免将“可解压”误解为“可自由创建 RAR”。

## 10. 主要资料链接

本方案参考的主要资料包括：GPUI README、Zed 主仓库、Zed releases、gpui-component、minizip-ng、sevenz-rust2、libarchive、libarchive encrypted 7z/RAR issue、unrar.rs、RARLAB downloads、RARLAB addons、RARLAB license、7-Zip SDK、7-Zip GitHub、7-Zip-zstd、NanaZip、image docs、fast_image_resize、zune-image、kamadak-exif、windows-rs、objc2、ashpd、rfd、notify-rust 和 tray-icon。
